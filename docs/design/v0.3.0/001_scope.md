...
| アイデンティティマップ | 物理 = 仮想 のストレートマッピング | |
| カーネル空間マップ | 上位アドレス `0xFFFF8000_00000000` にカーネル配置 | |
| CR3設定 | `mov cr3, pml4_addr` でページテーブル適用 | |
| ページフォルトハンドラ | ISR #14 でフォルト情報をデバッグ出力 | |
...
| `kmalloc()` / `kfree()` | カーネル内アロケータAPI | |
| `__cm_alloc` 実装 | Cm no_std 契約の実装 | |
...
- [ ] UEFIメモリマップからConventionalMemory領域を正しく解析できる
- [ ] ビットマップPMMでページ割当/解放が動作する
- [ ] アイデンティティマップのページテーブルを構築し、CR3にロードできる
- [ ] Bump Allocatorで`kmalloc()`が動作する
- [ ] ページフォルト発生時にフォルトアドレス（CR2）をシリアル出力
- [ ] `make test` で既存テストが引き続きパスする
- [ ] メモリ統計情報（総メモリ/空きメモリ）をシリアル出力
