...
> CosmOS開発中に発見されたCmコンパイラの制約・バグとその回避策をまとめたドキュメント。
> 対象: `--target=uefi` (x86_64)、macOS環境

## v0.2.0 修正状況

> [!IMPORTANT]
> Cm v0.2.0 (2026-02-17検証) で以下のバグがネイティブJITでは**全て修正確認済み**。
> ただしUEFIターゲットでは `0xFFFFFFFFFFFFFFFF` の二項演算直接使用で問題が残る場合があり、
> 変数経由での使用を推奨。

| # | バグ | JIT | UEFI |
|---|------|-----|------|
| 1 | `__asm__`出力変数のwhile条件 | ✅ | 要検証 |
| 2 | 整数リテラル型推論 | ✅ | ✅ |
| 3 | `stoll: out of range` | ✅ | ⚠️ 変数経由推奨 |
| 4 | `must { __asm__() }` | ✅ | 要検証 |
| 5 | const式計算 | ✅ | 要検証 |
| 7 | スタックオフセット重複 | ✅ | 要検証 |
